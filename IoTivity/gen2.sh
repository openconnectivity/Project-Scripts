...
MY_COMMAND="cp -f $CURPWD/device_output/code/server_introspection.dat $CURPWD/bin/"
eval ${MY_COMMAND}
...
then
  ...
  MY_COMMAND="cp -i $CURPWD/device_output/code/server.cpp $CURPWD/src/$PROJNAME.cpp"
  eval ${MY_COMMAND}
  MY_COMMAND="cp -i $CURPWD/device_output/code/server.cpp $CURPWD/src/$PROJNAME.cpp.gen"
  eval ${MY_COMMAND}
fi
